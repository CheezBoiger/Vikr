...
public:
  ...
  /// Insert a new RenderTarget type, this is generated by the Framebuffer.
  /// If a RenderTarget already occupies the point index, then it will overwrite 
  /// the previous RenderTarget. Framebuffers always retain ownership with RenderTargets,
  /// and can not release them!
  virtual vvoid InsertNewRenderTarget(RenderTargetType type, vuint32 point) = 0;

  /// Get the RenderTarget at that specific index point.
  virtual RenderTarget *GetRenderTarget(vuint32 point) = 0;

  /// Remove a RenderTarget from the Framebuffer, this will ultimately delete the
  /// RenderTarget, instead of giving up ownership.
  virtual vbool RemoveRenderTarget(vuint32 point) = 0;

  /// Get the number of rendertargets in this framebuffer.
